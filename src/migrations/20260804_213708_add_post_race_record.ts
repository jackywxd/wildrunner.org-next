...
/**
 * THE TIMESTAMP IS DELIBERATELY NOT THIS FILE'S CREATION TIME.
 *
 * An earlier attempt at this feature generated a migration under exactly
 * this name, and a build ran it against staging and production before the
 * branch was reverted. The branch went; the databases kept the columns, the
 * indexes, and the `20260804_213708_add_post_race_record` row in
 * `payload_migrations`. Regenerating under a fresh timestamp would give
 * Payload a migration it believes is pending, and its first statement —
 * `ALTER TABLE posts ADD race_record_id` — would fail on a column that is
 * already there, on both environments.
 *
 * So the name is matched to the ledger on purpose. Both databases skip this
 * file; a fresh one applies it and ends up in the same state. Do not
 * "correct" the timestamp to when this file was written.
 */

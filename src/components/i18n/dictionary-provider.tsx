 ...
 *
 * WHY THIS EXISTS AT ALL. 109 of the public site's 204 Chinese strings are
 * inside Client Components — the lightbox, the share sheet, the reader, the
 * media grid and its filters. `getDictionary()` cannot help them: it reads
 * the route with `next/root-params`, which is Server-Component-only. So the
 * server half resolves the dictionary once, in `(public)/layout.tsx`, and
 * hands it across the boundary here.
 *
 * IT COSTS NO BYTES THE PAGE WAS NOT ALREADY PAYING — near enough. Those 109
 * strings ship to the browser today, compiled into the component chunks that
 * contain them. This moves them out of the chunks and into the layout's

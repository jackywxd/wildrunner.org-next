...
> ✅ **已于 2026-07-28 完成**（Worker version `d39ec359`）。
> 下方步骤保留为后续手动部署与回滚的操作依据。
...
|------|--------|--------|
| `/admin` | 404 | **200** |
| `/api/posts` | 404 | **200**（9 篇已发布） |
| 内容来源 | 建置时烤进 bundle | **D1** |
| 首页 hero | — | 心如野馬，馳騁天下 |
| 文章内文图片 | — | 15/15 显示，全部经 Cloudflare 优化 |
| 相簿影片 | — | 200，R2 直接播放 |
| 帐号 | — | 只剩 `xudong.wu@gmail.com`（admin，byline 追雲逐雪） |

只读冒烟测试 6/6 通过。`P2-T1` 会失败且**这是预期的**：它需要登入建立文章，
而测试帐号已按步骤 2 从生产删除。生产不应存在密码公开的帐号。

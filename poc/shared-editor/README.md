...
- 步驟 2 的「語法樹相同」對 11 篇有軟換行的檔案，建立在 v1 的對應（換行節點 ⇄ 原始碼換行）上。
  設計 v2 改了規則（`docs/shared-editor.md` §4.5），被編輯的段落會合併成一行；未編輯的段落不受影響，
  所以步驟 2b 的 108 頁結果不變。正式版的測試要以步驟 2b（建置後的 HTML）為準，不以語法樹為準。
